...
# %%
# Load dataset from local directory (no network needed)
print("Loading dataset from local directory: data/commonsense_qa")
dataset = load_from_disk('data/commonsense_qa')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--small_model_id', type=str, 
                       ...
                       help='Small model path or name')
    parser.add_argument('--large_model_id', type=str, 
                       default="/home/lcw/.cache/huggingface/hub/models--Qwen--Qwen2.5-32B-Instruct/snapshots/5ede1c97bbab6ce5cda5812749b4c0bdf79b18dd", 
                       help='Large model path or name')
...
# print("Loading models to device1...")
# small_model = FSDAutoModelForCausalLM.from_pretrained(small_model_id, torch_dtype=torch.bfloat16).to(device1)
# # model = small_model
# model = FSDAutoModelForCausalLM.from_pretrained(large_model_id, torch_dtype=torch.bfloat16, device_map={"": device1})
print("Loading models in INT8 precision...")
print("Loading small model (7B) to device1 with INT8...")
small_model = FSDAutoModelForCausalLM.from_pretrained(
    small_model_id, 
    load_in_8bit=True,
    device_map={"":  device1}
)

print("Loading large model (32B) with INT8 and auto device mapping...")
model = FSDAutoModelForCausalLM.from_pretrained(
    large_model_id, 
    load_in_8bit=True,
    device_map={"":  device1}
)
...
def eval_commonsenseqa(model, small_model, fsd_div_threshold, fsd_div_type, dataset, tokenizer=tokenizer, device=device1, do_sample=do_sample, top_k=top_k, top_p=top_p, temperature=temperature, eval_num=0, exp=exp, flash_sd_entropy_scale=0.0, flash_sd_lookahead=0.0):
    ...
    with torch.no_grad():
        for i, item in tqdm(enumerate(dataset), total=len(dataset)):
            ...
            question_tokenized = tokenizer(formatted_question, return_tensors='pt').to(device)
